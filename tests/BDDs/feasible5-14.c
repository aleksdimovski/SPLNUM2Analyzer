features int[0,4] A1;
features int[0,4] A2;
features int[0,4] A3;
features int[0,4] A4;
features int[0,4] A5;
features int[0,4] A6;
features int[0,4] A7;
features int[0,4] A8;
features int[0,4] A9;
features int[0,4] A10;
features int[0,4] A11;
features int[0,4] A12;
features int[0,4] A13;
features int[0,4] A14;

int main() {
  int i=0;

  #if (A1==0) i=i+1; #else i=0; #endif
  #if (A2==0) i=i+1; #else i=0; #endif
  #if (A3==0) i=i+1; #else i=0; #endif
  #if (A4==0) i=i+1; #else i=0; #endif
  #if (A5==0) i=i+1; #else i=0; #endif
  #if (A6==0) i=i+1; #else i=0; #endif
  #if (A7==0) i=i+1; #else i=0; #endif	
  #if (A8==0) i=i+1; #else i=0; #endif
  #if (A9==0) i=i+1; #else i=0; #endif
  #if (A10==0) i=i+1; #else i=0; #endif		
  #if (A11==0) i=i+1; #else i=0; #endif
  #if (A12==0) i=i+1; #else i=0; #endif
  #if (A13==0) i=i+1; #else i=0; #endif
  #if (A14==0) i=i+1; #else i=0; #endif
	
  return 0;
}