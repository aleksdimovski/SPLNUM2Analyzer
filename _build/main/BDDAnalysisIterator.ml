...
(* IMPORTANT FUNCTION THAT DOES THE ANALYSIS*)
  let analyze (vars,stmts,funcs) main =
	...
	StringMap.iter (fun key value -> list_feat := key :: !list_feat; list_featV := { varId = value.featId; varName = value.featName; varTyp = value.featTyp } :: !list_featV ) !ItoA.features; list_configs := process !list_feat; 
