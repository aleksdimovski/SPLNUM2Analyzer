...
let rec aInterval_to_bddapron env cond e =
  match e with
  | A_interval (i1,i2) -> Expr1.Apron.cst env cond (Coeff.s_of_int i1)
...
let rec exp_print fmt e =
  match e with
  | A_arithmetic a -> aExp_print fmt a
  | A_boolean b -> bExp_print fmt b
  | A_stmt -> Format.fprintf fmt "stmt"
